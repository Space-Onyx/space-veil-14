...
public sealed partial class ReverseBearTrapSystem : EntitySystem
{
    ...
    private void OnEquipped(EntityUid uid, ReverseBearTrapComponent trap, GotEquippedEvent args)
    {
        ...
        ArmTrap(uid, trap, args.EquipTarget);
    }
        ...
        if (trap.Ticking && trap.Wearer.HasValue)
        {
            ...
            if (args.User == trap.Wearer)
            {
                args.Verbs.Add(new Verb()
                {
                    ...
                    Text = Loc.GetString("reverse-bear-trap-verb-escape")
                });
            }
            else
            {
                ...
                {
                    DoContactInteraction = true,
                    Text = Loc.GetString("reverse-bear-trap-verb-remove"),
                    Disabled = !activeItem.HasValue || !_tools.HasQuality(activeItem.Value, WeldingQuality),
                    Act = () =>
                    {
                        var user = args.User;
                        ...
                        _tools.UseTool(activeItem!.Value, args.User, uid, 5f, new List<ProtoId<ToolQualityPrototype>> { "Welding" }, new WeldFinishedEvent(), 3f);
                    }
                });
            }
            ...
            if (activeItem.HasValue
                ...
                && _tag.HasTag(tagComponent, TrapKeyTag))
            {
                args.Verbs.Add(new Verb()
                {
                    DoContactInteraction = true,
                    Text = Loc.GetString("reverse-bear-trap-verb-unlock"),
                    Act = () =>
                    {
                        var user = args.User;
                        var target = trap.Wearer.Value;
    ...
    private void OnWeldFinished(EntityUid uid, ReverseBearTrapComponent trap, WeldFinishedEvent args)
    {
        ...
        _damageable.TryChangeDamage(trap.Wearer.Value, damage, true, origin: args.Used);
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var trap))
        {
            ...
            var remaining = trap.CountdownDuration - (float) (_timing.CurTime - trap.ActivateTime).TotalSeconds;
            if (remaining <= 0)
            {
                SnapTrap(uid, trap);
                continue;
    ...
    private void ArmTrap(EntityUid uid, ReverseBearTrapComponent trap, EntityUid wearer)
    {
        ...
        trap.Ticking = true;
        trap.ActivateTime = _timing.CurTime;
        trap.Wearer = wearer;
    ...
    private void SnapTrap(EntityUid uid, ReverseBearTrapComponent? trap)
    {
        ...
        _damageable.TryChangeDamage(wearer.Value, damage, true, origin: uid);
    }
    ...
    private static readonly string WeldingQuality = "Welding";
    private static readonly ProtoId<TagPrototype> TrapKeyTag = "ReverseBearTrapKey";
}